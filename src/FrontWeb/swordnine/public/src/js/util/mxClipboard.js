...
var mxClipboard =
{
	...
	setCells: function(cells)
	{
		...
		for (let index = 0; index < cells.length; index++) {
			...
			element.dataId = ''
			element.sntype = -1
			element.geometry.height = 60
			element.geometry.x = element.geometry.x + element.geometry.width
			if (element.children && element.children.length > 0) {
				element.children = [element.children[0]]
			}
			this.initCopyData(element.children)
		}
	...
	copy: function(graph, cells)
	{
		...
		mxClipboard.insertCount = 1;
		var copyCells = graph.cloneCells(result)
		mxClipboard.setCells(copyCells);
