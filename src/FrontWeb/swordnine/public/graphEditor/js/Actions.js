...
Actions.prototype.init = function()
{
	...
	// Edit actions
	this.addAction('undo', function() { return false; ui.undo(); }, null, 'sprite-undo', null, Editor.ctrlKey + '+Z');//DELETE:Editor.ctrlKey + '+Z'
	this.addAction('redo', function() { return false; ui.redo(); }, null, 'sprite-redo', null, (!mxClient.IS_WIN) ? Editor.ctrlKey + '+Shift+Z' : Editor.ctrlKey + '+Y');//DELETE:(!mxClient.IS_WIN) ? Editor.ctrlKey + '+Shift+Z' : Editor.ctrlKey + '+Y'
	...
	function deleteCells(includeEdges)
	{
		...
		var ret = window.deleteCells(selCells)
		if (ret === false) {
			return false
		}
		var select = graph.deleteCells(graph.getDeletableCells(selCells), includeEdges);
	...
	//自定义动作
    this.addAction('saveDraw',function () {
		window.sendSaveData()
